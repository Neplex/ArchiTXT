...
def schema_st(
    ...
    *,
    ...
    if entities is None:
        entities = draw(st.sets(LABEL_ST, min_size=2, max_size=15))
...
def entity_tree_st(
    ...
    if schema and schema.entities:
        entities = list(schema.groups.get(group_name, schema.entities))
...
def tree_st(draw: st.DrawFn, *, schema: Schema | None = None, has_parent: bool | None = None) -> Tree:
    ...
    tree = draw(
        st.recursive(
            base_strategy,
            lambda children: st.builds(Tree, st.just(''), st.lists(children, min_size=1, max_size=5)),
            max_leaves=5,
        )
    )
