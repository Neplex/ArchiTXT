...
Rules:
- Do NOT modify ENT nodes; keep them unchanged.
...
def _tree_to_list(trees: Iterable[Tree]) -> str:
    return '\n'.join(
        f'{i}. {json.dumps(tree.to_json())}' for i, tree in enumerate(trees, start=1) if isinstance(tree, Tree)
    )
