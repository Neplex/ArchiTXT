...
- Use generic group name not specific ones.
{vocab}
...
def count_tokens(llm: BaseLanguageModel, prompt: BasePromptTemplate, trees: Iterable[Tree], vocab: str = '') -> int:
    """
    ...
    """
    text = prompt.format(trees=_tree_to_list(trees), vocab=vocab or '')
...
def estimate_tokens(
    ...
    *,
    ...
    batches = more_itertools.constrained_batches(
        trees,
        max_size=max_tokens,
        get_len=lambda x: count_tokens(llm, prompt, x),
        strict=True,
    )

    prompt_tokens = sum(count_tokens(llm, prompt, batch) for batch in batches)
    return int(prompt_tokens * (refining_steps + 1) * error_adjustment)
...
async def llm_simplify(
    ...
    batches = more_itertools.constrained_batches(
        trees,
        max_size=max_tokens,
        get_len=lambda x: count_tokens(llm, prompt, x, vocab_str),
        strict=True,
    )
    ...
    with mlflow.start_span('rewriting') if mlflow.active_run() else nullcontext():
        for iteration in trange(refining_steps + 1, leave=False, desc='rewriting iterations'):
            with (
                ...
                simplification = llm_simplify(llm, max_tokens, prompt, simplification, debug, vocab)
