...
@st.cache_resource
def get_benepar_parser(spacy_models: dict[str, str]) -> BeneparParser:
    return BeneparParser(spacy_models=spacy_models)
...
def _render_text_corpus_import(forest: TreeBucket) -> None:
    ...
    with st.expander("Advanced NLP Settings"):
        ...
        edited_mapping_df = st.data_editor(mapping_df, width='stretch', num_rows="dynamic")
    ...
    if st.button("Load Corpus", disabled=not uploaded_files):
        ...
        st.success(f"Loaded {len(forest)} instances.")
        st.rerun()
    ...
    if st.button("Load document", disabled=not uploaded_documents):
        with st.spinner("Loading from document..."):
            for document in uploaded_documents:
                trees = read_document(document, root_name=document.name)
                forest.update(trees, commit=True)
...
def _render_jsonl_import(forest: TreeBucket) -> None:
    ...
    if st.button("Load JSONL", disabled=not uploaded_jsonls):
        with (
            st.spinner("Loading from JSONL..."),
            tempfile.TemporaryDirectory() as tmpdir,
        ):
            tmp_path = Path(tmpdir) / "upload.jsonl"

            for jsonl in uploaded_jsonls:
                tmp_path.write_bytes(jsonl.getvalue())
                trees = import_forest_from_jsonl(tmp_path)
                forest.update(trees, commit=True)
