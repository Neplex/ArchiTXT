...
def _render_rule_based_simplification(forest: TreeBucket) -> None:
    c1, c2 = st.columns(2)
    tau = c1.number_input('Tau (Similarity)', 0.0, 1.0, 0.7)
    decay = c2.number_input('Decay', 0.0, 1.0, 0.5)
    min_support = c1.number_input('Min Support', 1, 100, 10)
    epoch = c2.number_input('Epochs', 1, 1000, 100)
...
def _render_llm_based_simplification(forest: TreeBucket) -> None:
    ...
    with st.expander("Advanced LLM Settings"):
        ...
        max_tokens = c2.number_input("Max Tokens", 256, 65536, 4096)
        refining_steps = c3.number_input("Refining Steps", 1, 10, 3)

        tau = c1.number_input('Tau (Similarity)', 0.0, 1.0, 0.7)
        decay = c2.number_input('Decay', 0.0, 1.0, 0.5)
        min_support = c3.number_input('Min Support', 1, 100, 10)
