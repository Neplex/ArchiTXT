...
def test_find_relations_simple() -> None:
    ...
    operation = FindRelationsOperation(tree_clusterer=clusterer, min_support=0)
...
def test_find_relations_naming_only() -> None:
    ...
    clusterer = TreeClusterer(tau=0.0, metric=jaccard)
    operation = FindRelationsOperation(tree_clusterer=clusterer, min_support=0, naming_only=True)
    has_simplified = operation.apply(tree)
